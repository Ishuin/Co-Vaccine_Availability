...
class SlotsSyncConsumer(AsyncConsumer):
    ...
    async def websocket_receive(self, event):
        print("-------------- receive --------------------")
        scope_headers = {
            x[0].decode("utf-8"): x[1].decode("utf-8") for x in self.scope["headers"]
        }
        today = datetime.datetime.now().date()
        dates = [
            "-".join(str(today + datetime.timedelta(days=i)).split("-")[::-1])
            for i in range(1, 8)
        ]
        ...
        while True:
            for date in dates:
                time.sleep(3)
                list_of_dicts = sync_to_async(self.slot_data)(
                    date, district_id, scope_headers
                )
                dicts = await list_of_dicts
                await self.send(
                    {"type": "websocket.send", "text": json.dumps({"date": date})}
                )
                for x in dicts:
                    ...
                    if (
                        x["min_age_limit"] == 18
                        and x["available_capacity_dose2"] > 0
                        and x["fee_type"] != "Paid"
                        and x["vaccine"] == "COVISHIELD"
                    ):
                        await self.send(
                            {"type": "websocket.send", "text": json.dumps(x)}
                        )
